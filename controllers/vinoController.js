...
exports.pretragaVina = async (req, res) => {
  const svavina = await Vino.find();
  res.render("pretraga", {
    title: "Pretrazivanje vina",
    svavina,
  });
};
...
exports.snimiUredjenoVino = async (req, res) => {
  ...
  req.flash(
    "success",
    `Uspješno ste uredili vino <strong>${vino.naziv}</strong>!`
  );
  res.redirect("/");
};
...
exports.mojaKolekcijaVina = async (req, res) => {
  ...
  const vina = await Vino.find({ ime: loggedUser })
    .collation({ locale: "hr", strength: 2 })
    .sort({ zemlja: 1 });
  // .skip(skip)
  // .limit(limit);
...
exports.ukloniVino = async (req, res) => {
  ...
  if (
    (loggedUser &&
      loggedUser._id.toString() === vinoToDelete.korisnik.toString()) ||
    loggedUser.level === 1
  ) {
    ...
    req.flash(
      "success",
      `Uspješno ste uklonili vino <strong>${vino.naziv}</strong>`
    );
    res.redirect("/");
  } else {
